...
# Cria config.txt se nao existir
if not os.path.isfile(f'{os.getcwd()}\\config.txt'):
    with open('config.txt', 'w', encoding='utf_8') as arq: # Cria arquivo config.txt e abre em modo escrita
        arq.write('# Download path\n')
        arq.write(f'download_path={os.getcwd()}\n') # This path
        arq.write('# Username\n')
        arq.write('username=\n')
        arq.write('# Password\n')
        arq.write('password=')
...
with open(f'{os.getcwd()}\\config.txt', encoding='utf_8') as arq: # Abre arquivo em modo somente leitura
    lines = arq.readlines() # Le linha por linha
    arq.close() # Fecha arquivo

# Percorre linhas de config.txt
for line in lines:
