#!/usr/bin/env python
# ADP-ClockIn
# Desenvolvido por Marcos Roberto Baungartner
# v1.7 Final
...
if not os.path.isfile('path.txt'):
    arq=open('path.txt', 'w', encoding='utf_8') # Cria arquivo path.txt e abre em modo escrita
    arq.write('# Driver path\n')
    arq.write(f'driver_path={os.path.expanduser("~")}\n') # C:\Users\<User>
    arq.write('# Download path\n')
    arq.write(f'download_path={os.path.expanduser("~")}') # C:\Users\<User>
    arq.close() # Fecha arquivo

arq=open('path.txt', encoding='utf_8') # Abre arquivo em modo somente leitura
lines = arq.readlines() # Le linha por linha
arq.close() # Fecha arquivo

# Percorre linhas de path.txt
for line in lines:
    if line.find('driver_path=') >= 0: # Se encontrar variavel
        driver_path = (line.rsplit('driver_path=', 1)[-1]).strip() # Recebe valor da variavel
        driver_version_latest = '0'
        for f in glob.glob(f'{driver_path}\\**', recursive=False): # Percorre diretorios
            driver_version = f.rsplit('\\', 1)[-1] # Guarda apenas o que vem depois do ultimo \\
            if int(driver_version.split('.', 1)[0]) > int(driver_version_latest.split('.', 1)[0]):
                driver_version_latest = driver_version
...
def login():
    ...
    entry_user.insert(0, '') # Inserir User (opcional)
    ...
    entry_password.insert(0, '') # Inserir Password (opcional)
...
def marcar_ponto(driver, mes_ano, dia_mes, entrada1, saida1, entrada2, saida2, forcar_marcacao):
    ...
    try:
        ...
        WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, '//*[normalize-space()="%s"]' %dia_mes))).click()
...
def login_adp(username, password, chk_box):
    ...
    try: #Configuracao do chromedriver
        ...
        driver = webdriver.Chrome(service=Service('C:/Users/Beto/.wdm/drivers/chromedriver/win32/'+driver_version_latest+'/chromedriver.exe'), options=chrome_options)
        ...
        try: #Sessão do ADP
            ...
            if chk_box == 1: # Se Checkbutton 'Baixar Demonstrativo' estiver selecionado
                ...
                try:
                    for i in range(1,3):
                        WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.XPATH, '//*[@id="js-react-app"]/div/div/div[4]/div[2]/div[2]/div[1]/div/div/div[1]/ol/li[%i]/a/div' %i))).click() # Acessa pagamento
                        nome_pagamento = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.XPATH, '//*[@id="js-react-app"]/div/div/div[4]/div[2]/div[2]/div[1]/div/div/div[1]/ol/li[%i]/a/div/h3/strong' %i))).get_attribute('innerHTML') # Recebe nome do pagamento
                        data_pagamento = WebDriverWait(driver, 10).until(EC.visibility_of_element_located((By.XPATH, '//*[@id="js-react-app"]/div/div/div[4]/div[2]/div[2]/div[1]/div/div/div[1]/ol/li[%i]/a/div/dl/dd[2]' %i))).get_attribute('innerHTML').split('/') # Recebe data de pagamento
            ...
            try: # Se nao houve erro
                ...
                for row in range(1,ws.max_row + 1): # Percorre linhas da planilha
                    if ws['H' + str(row)].value is None or ws['H' + str(row)].value == 'MARCAR': # Se Status estiver vazio ou MARCAR
                        ...
                        if data.date() < date.today(): # Se data da planilha for menor que data de hoje
                            try:
                                ...
                                result = marcar_ponto(driver, mes_ano, dia_mes, entrada1, saida1, entrada2, saida2, forcar_marcacao) # Marca o ponto
                                if result == 'OK':
                                ...
                                if result != 'ERRO': # Se não houver ERRO
                                    ws['H' + str(row)].value = result # Se ponto marcado, marca OK/FOLGA/DSR/FERIADO/ABONO na celula Status
                                    wb.save('ADP.xlsx') # Salva planilha
